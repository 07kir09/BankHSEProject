...
using AccountingForFinances.Commands;
using YamlDotNet.Serialization;
using YamlDotNet.Serialization.NamingConventions;
...
namespace BankHSEProject.ConsoleApp
{
    public class AppRunner
    {
            ...
            while (!exit)
            {
                ...
                switch (choice)
                {
                    ...
                    case "5":
                        ...
                        break;
                    case "6":
                        // Новый пункт меню для создания банковского счёта.
                        ...
                        ICommand createAccountCommand = new TimeMeasurementCommandDecorator(
                            new CreateBankAccountCommand(_bankAccountFacade, bankName, initialBalance),
        ...
        void DisplayMenuOptions()
        {
            ...
            Console.WriteLine("4. Экспорт данных");
            Console.WriteLine("5. Выход");
            Console.WriteLine("6. Создать банковский счёт");
            Console.ResetColor();
            Console.WriteLine("────────────────────────────────────────────────────────");
        }

        // Метод чтения ввода пользователя.
        private string ReadUserInput()
        {
            try
            {
        ...
        void RunAnalytics()
        {
            ...
            try
            {
                // Используем AnalyticsFacade для вычислений
                decimal diff = _analyticsFacade.GetIncomeExpenseDifference(financeData.operations, startDate, endDate);
                var grouped = _analyticsFacade.GroupByCategory(financeData.operations, financeData.categories);
                var monthlyStats = _analyticsFacade.GetMonthlyStats(financeData.operations);
        ...
        void BuildFinanceDataSnapshot()
        {
            financeData = new FinanceData
            {
                bankAccounts = new List<BankAccount>(_bankAccountFacade.GetAllBankAccounts()),
                categories = new List<Category>(_categoryFacade.GetAllCategories()),
                operations = new List<Operation>(_operationFacade.GetAllOperations())
            };
        }
